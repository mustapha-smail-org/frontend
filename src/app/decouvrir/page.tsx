...
export default async function DiscoverPage({ searchParams }: Props) {
  ...
  let events: CursorPage<EventSummary> = { items: [], nextCursor: null, hasNext: false }; let markers: EventMapMarker[] = []; let categories: string[] = []; let initialError = false;
  try { [events, { items: markers }, categories] = await Promise.all([getEvents({ ...options, limit: 18 }), getMapEvents({ ...options, limit: 100 }), getCategories()]); } catch { initialError = true; }
  return <><section className="discover-title shell-pad"><div className="mx-auto max-w-[88rem]"><p className="eyebrow">Explorer Paris</p><h1>Qu’est-ce qui vous<br/><em>fait sortir ?</em></h1></div></section><DiscoveryExperience initialEvents={events} initialMarkers={markers} categories={categories} initialError={initialError}/></>;
}
